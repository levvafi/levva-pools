...
import '../../router/adapters/BalancerAdapter.sol';
...
contract TestVault is IVault {
  ...
  ) private view returns (uint256 amountCalculated, uint256 amountIn, uint256 amountOut) {
  ...
  function _processGeneralPoolSwapRequest(
    ...
    _getAmounts(request.kind, request.amount, amountCalculated);
  }
  ...
  ) internal view returns (bytes32 newTokenInBalance, bytes32 newTokenOutBalance, uint256 amountCalculated) {
    newTokenInBalance = bytes32(tokenInBalance);
    newTokenOutBalance = bytes32(tokenOutBalance);
    amountCalculated = onSwap(request);
    _getAmounts(request.kind, request.amount, amountCalculated);
  }
