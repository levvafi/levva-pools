...
contract PendleCurveRouterNgAdapter is IMarginlyAdapter, Ownable2Step {
    ...
    struct RouteInput {
        ...
        uint8 slippage;
        int32 curveDxAdjustPtToToken; // by default 0.001%, 0.00001
        int32 curveDxAdjustTokenToPt;
    ...
    int256 private constant CURVE_DX_ADJUST_ONE = 1e6;
    ...
    constructor(address _curveRouter, RouteInput[] memory routes) Ownable(msg.sender) {
    ...
    function swapExactInput(
        ...
        if (routeData.pt.isExpired()) {
            ...
            amountOut =
                _swapExactInputPreMaturity(routeData, recipient, tokenIn, tokenOut, amountIn, minAmountOut, data);
        }
    ...
    function swapExactOutput(
        ...
        if (routeData.yt.isExpired()) {
            ...
            amountIn =
                _swapExactOutputPreMaturity(routeData, recipient, tokenIn, tokenOut, maxAmountIn, amountOut, data);
        }
        ...
        if (syToAccount > 0) {
            ...
            if (data.isExactOutput) {
                ...
                uint256 estimatedQuoteAmount = ICurveRouterNg(_curveRouter).get_dx(
                    routeData.curveRoute, routeData.curveSwapParams, ibAmount, routeData.curvePools
                );
                estimatedQuoteAmount = (estimatedQuoteAmount * uint256((CURVE_DX_ADJUST_ONE + routeData.curveDxAdjust)))
                    / uint256(CURVE_DX_ADJUST_ONE);

                callbackAmountIn = estimatedQuoteAmount;

                IMarginlyRouter(data.router).adapterCallback(
                    address(this), estimatedQuoteAmount, data.adapterCallbackData
                );
    ...
    function _swapExactInputPreMaturity(
        ...
        if (tokenIn == address(routeData.pt)) {
            ...
            amountOut = _pendleApproxSwapExactSyForPt(
                routeData, recipient, ibAmount, minAmountOut, abi.encode(swapCallbackData)
            );
        }
    }
    ...
    function _swapExactOutputPreMaturity(
        ...
        if (tokenIn == address(routeData.pt)) {
            ...
            estimatedIbAmount = (estimatedIbAmount * uint256((CURVE_DX_ADJUST_ONE + routeData.curveDxAdjust)))
                / uint256(CURVE_DX_ADJUST_ONE);
            ...
            (, uint256 ptAmountIn) = _pendleApproxSwapPtForExactSy(
                routeData, address(this), estimatedIbAmount, maxAmountIn, abi.encode(swapCallbackData)
            );
            amountIn = ptAmountIn;
    ...
    function _swapExactInputPostMaturity(
        ...
        if (tokenIn == address(routeData.pt)) {
            ...
            amountOut = ICurveRouterNg(_curveRouter).exchange(
                routeData.curveRoute, routeData.curveSwapParams, ibAmount, minAmountOut, routeData.curvePools, recipient
            );
        } else {
    ...
    function _pendleMintSy(RouteData memory routeData, address recipient, uint256 ibIn)
        private
        returns (uint256 syMinted)
    {
    ...
    function _pendleRedeemSy(RouteData memory routeData, address recipient, uint256 syIn)
        private
        returns (uint256 ibRedeemed)
    {
        // setting `minTokenOut` value as syIn (1:1 swap)
        ibRedeemed = routeData.sy.redeem(recipient, syIn, routeData.ib, syIn, false);
    }

    function _redeemPY(IPYieldToken yt, address router, uint256 ptAmount, bytes memory adapterCallbackData)
        private
        returns (uint256 syRedeemed)
    {
    ...
    function _addPairs(RouteInput[] memory routes) private {
        ...
        for (uint256 i; i < length;) {
            ...
            if (input.curveDxAdjustPtToToken.abs() >= uint256(CURVE_DX_ADJUST_ONE)) revert WrongInput();
            if (input.curveDxAdjustTokenToPt.abs() >= uint256(CURVE_DX_ADJUST_ONE)) revert WrongInput();
...

            {
                RouteData memory ptToQuoteSwapRoute = RouteData({
                    ...
                    curveDxAdjust: input.curveDxAdjustPtToToken,
                ...
                RouteData memory quoteToPtSwapRoute = RouteData({
                    ...
                    yt: yt,
                    slippage: input.slippage,
                    curveDxAdjust: input.curveDxAdjustTokenToPt,
