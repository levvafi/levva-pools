...
contract ZyberAdapter is AdapterStorage, UniswapV2LikeSwap {
  using LowGasSafeMath for uint256;

  uint256 private constant ZYBER_FEE = 9975;
  ...
  function swapExactInput(
    ...
    amountOut = getAmountOut(pool, amountIn, tokenIn, tokenOut, ZYBER_FEE);
  ...
  function swapExactOutput(
    ...
    address pool = getPoolSafe(tokenIn, tokenOut);
    amountIn = getAmountIn(pool, amountOut, tokenIn, tokenOut, ZYBER_FEE);
  ...
  function getAmountOut(
    ...
    (uint256 reserveIn, uint256 reserveOut) = getZyberSwapReserves(pool, tokenIn, tokenOut);
