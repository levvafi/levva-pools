...
export async function longAndShort(sut: SystemUnderTest) {
  ...
  for (let i = 0; i < longersNumber; ++i) {
    ...
    logger.info(`longer: ${longer.address}`);
    const longAmount = BigNumber.from(4n * 10n ** 18n).add(BigNumber.from(1n * 10n ** 16n)*(i)); // 4 WETH + 0.01WETH * i
    ...
    longersAmounts[i][1] = longersAmounts[i][1].add(swapAmount*(swapMultiplierLong)/(FP96.one));
  ...
  for (let i = 0; i < shortersNumber; ++i) {
    ...
    logger.info(`shorter: ${shorter.address}`);
    const shortAmount = BigNumber.from(4n * 10n ** 18n).add(BigNumber.from(1n * 10n ** 16n)*(i)); // 5 WETH + 0.01WETH * i
    ...
    shortersAmounts[i][1] = shortersAmounts[i][1].add(swapAmount*(swapMultiplierShort)/(FP96.one));
  ...
  let delta = shortersTotalDebtDelta-(longersTotalCollDelta.add(realBaseDebtFee)).abs();
  if (delta > epsilon) {
  ...
  delta = longersTotalDebtDelta-(shortersTotalCollDelta.add(realQuoteDebtFee)).abs();
  if (delta > epsilon) {
  ...
  logger.warn(`basePrice: ${toHumanString(BigNumber.from((await marginlyPool.getBasePrice()).inner)*(10 ** 12))}`);
