...
import { IUniswapV3Pool, IUSDC, IWETH9 } from '../../../typechain-types';
...
export async function changeWethPrice(
  ...
  const wethPrice = BigInt(await estimateTickToPrice(weth, usdc, Number(tick)));
  ...
  while (decreasingPrice ? currentPrice > targetPrice : targetPrice > currentPrice) {
    ...
    const { tick } = await uniswap.connect(treasury.provider).slot0();
    const price = BigInt(await estimateTickToPrice(weth, usdc, Number(tick)));
...
async function estimateTickToPrice(
  ...
  const factor = Math.pow(10, Number(baseDecimals - quoteDecimals));

  return Math.floor(
    baseToken.toString().toLowerCase() < quoteToken.toString().toLowerCase() ? price / factor : factor / price
  );
}
